...
a placeholder. **Profile's main screen, "Mes préférences", "Mes favoris", "Mon historique" and "Mes statistiques"
are now real** (header, stats, grouped menu; experience types, ambiance, budget/distance sliders; favorited
experiences with removal and an empty state; completed experiences grouped by date with a category filter;
summary cards, a genre/city breakdown and a mood donut chart (`react-native-gifted-charts`) — mock data only,
one screen at a time) — see `DECISIONS.md` D-50, D-51, D-57, D-58, D-59; the other screens it links to (edit
profile, language, theme, help, privacy, settings) are still placeholders, each built in its own session.
