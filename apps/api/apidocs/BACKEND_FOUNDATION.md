...
logging, CORS, health check, routing, OpenAPI and tests. **No domain module is implemented yet** (the data model exists since API-03, [`DATABASE_SCHEMA.md`](DATABASE_SCHEMA.md): Experiences, Places, Events,
...
├── database/                   DatabaseModule (global) + PrismaService (the only Prisma client), persistence
...
prisma/migrations/              init (generated by Prisma) + check_constraints (hand-written SQL), applied
...
test/database/                  PostgreSQL integration tests (pnpm test:db, vitest.db.config.ts)
```
...
Controllers never touch Prisma; repositories inject `PrismaService` (never `new PrismaClient()`) and return domain
types — the boundary is enforced by ESLint ([`REPOSITORY_ARCHITECTURE.md`](REPOSITORY_ARCHITECTURE.md)). Provider
adapters follow [`PROVIDER_ARCHITECTURE.md`](PROVIDER_ARCHITECTURE.md).
...
- `prisma/schema.prisma` declares the PostgreSQL datasource and the `prisma-client` generator (ESM, output
  `src/generated/prisma`) and, since API-03, the ROAM data model — see [`DATABASE_SCHEMA.md`](DATABASE_SCHEMA.md).
...
| Persistence error (repositories, API-04)                                           | 404 / 409 / 422 / 400 / 503 | `NOT_FOUND` / `CONFLICT` / `UNPROCESSABLE_ENTITY` / `BAD_REQUEST` / `DATABASE_UNAVAILABLE` | generic ([`REPOSITORY_ARCHITECTURE.md`](REPOSITORY_ARCHITECTURE.md) → "Errors") |
| Raw Prisma error (safety net: translated the same way; unreachable database → 503) | as above                    | as above                                                                                   | generic                                                                         |
| Anything else (bugs, other Prisma errors)                                          | 500                         | `INTERNAL_ERROR`                                                                           | generic                                                                         |

Stack traces are logged server-side and never sent; SQL, driver messages and row data are neither sent nor logged.
Repositories translate Prisma errors into persistence errors; services throw `ApiException` with their own codes
(e.g. `JOURNEY_ALREADY_ACTIVE`).
...
`pnpm --filter @roam/api test:db` — **database integration tests** on a real PostgreSQL, the dedicated database named
by `DATABASE_TEST_URL` (never the development one): migrations, constraints, delete rules, and the real app's health
endpoints. Setup and safety rules: [`DATABASE_SCHEMA.md`](DATABASE_SCHEMA.md) → "Tests".

## Local development

**Current local setup (2026-09-26):** Node.js 24, pnpm 12 and a **local PostgreSQL 18.6** (databases `roam` and
`roam_test` — [`DATABASE_SCHEMA.md`](DATABASE_SCHEMA.md) → "Local PostgreSQL"). **Docker is not used.**

Without PostgreSQL, everything but `test:db` and the migration commands still works: install, `prisma generate`/
`validate`, typecheck, lint, `test`, and running the API outside production (`/health` answers, `/health/database`
reports 503, the startup logs a warning).
...
## Required later for a deployed backend
